...
static int create_packet_v2_sock (int host_if_index, tpacket_req_t *rx_req,
...
int af_packet_create_if (vlib_main_t *vm, u8 *host_if_name, u8 *hw_addr_set,
                         u32 *sw_if_index, afpacket_interface_mode_t mode)
{
  af_packet_main_t *apm = &af_packet_main;
  int ret, fd = -1, fd2 = -1;
  ...
  if (mode == AF_PACKET_INTERFACE_MODE_IP)
    {
      apif->hw_if_index =
          vnet_register_interface (vnm, af_packet_device_class.index, 0,
                                   af_packet_device_class.index, 0);
    }
  else
    {
...
int af_packet_dump_ifs (af_packet_if_detail_t **out_af_packet_ifs)
{
  ...
  pool_foreach (apif, apm->interfaces, ({
                  ...
                  if (apif->host_if_name)
                    {
                      clib_memcpy (
                          af_packet_if->host_if_name, apif->host_if_name,
