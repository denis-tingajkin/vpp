...
static u32 memif_eth_flag_change (vnet_main_t *vnm, vnet_hw_interface_t *hi,
                                  u32 flags)
{
  /* nothing for now */
  return 0;
...
memif_get_ring (memif_if_t *mif, memif_ring_type_t type, u16 ring_num)
{
  ...
  int ring_size = sizeof (memif_ring_t) +
  ...
  for (i = 0; i < mif->run.num_m2s_rings; i++)
    {
      ...
      for (j = 0; j < (1 << mif->run.log2_ring_size); j++)
        {
          u16 slot =
              (i + mif->run.num_s2m_rings) * (1 << mif->run.log2_ring_size) +
              j;
          ring->desc[j].region = 0;
          ring->desc[j].offset =
              buffer_offset + (u32) (slot * mif->run.buffer_size);
...
static uword memif_process (vlib_main_t *vm, vlib_node_runtime_t *rt,
                            vlib_frame_t *f)
{
  memif_main_t *mm = &memif_main;
  memif_if_t *mif;
            ...
            if (mif->flags & MEMIF_IF_FLAG_IS_SLAVE)
              {
                ...
                sock->flags =
                    CLIB_SOCKET_F_IS_CLIENT | CLIB_SOCKET_F_SEQPACKET;
                ...
                if ((err = clib_socket_init (sock)))
                ...
                else
                  {
                    ...
                    t.description =
                        format (0, "%U ctl", format_memif_device_name,
                                mif->dev_instance);
                    hash_set (msf->dev_instance_by_fd, sock->fd,
                              mif->dev_instance);
...
int memif_socket_filename_add_del (u8 is_add, u32 sock_id, u8 *sock_filename)
{
  ...
  if (sock_filename[0] != '/')
  ...
  else
    {
      ...
      if ((dir == NULL) || (faccessat (/* ignored */ -1, dir,
                                       F_OK | R_OK | W_OK, AT_EACCESS) < 0))
        {
          vec_free (dir);
          return VNET_API_ERROR_INVALID_ARGUMENT;
        }
      ...
      if (args->is_master && (stat ((char *)msf->filename, &file_stat) == 0))
        {
          if (S_ISSOCK (file_stat.st_mode))
            ...
            {
              error =
                  clib_error_return (0, "File exists for %s", msf->filename);
  ...
  if (mif->mode == MEMIF_INTERFACE_MODE_ETHERNET)
    {
      ...
      error = ethernet_register_interface (
          vnm, memif_device_class.index, mif->dev_instance, args->hw_addr,
          &mif->hw_if_index, memif_eth_flag_change);
    }
  else if (mif->mode == MEMIF_INTERFACE_MODE_IP)
    {
      mif->hw_if_index = vnet_register_interface (
          vnm, memif_device_class.index, mif->dev_instance,
          memif_ip_hw_if_class.index, mif->dev_instance);
    }
  else
